...
var Navbar = React.createClass({displayName: 'Navbar',
	...
	componentDidMount: function() {
		if (this.state.scriptId != -1)
		{
			console.log("edit");
			this.getFromDatabase(this.state.scriptId);
		}

		console.log('component mounted. current script id is ' + this.state.scriptId);
	},
	...
	getFromDatabase: function(){
		...
		var id =this.state.scriptId;
		console.log('will receive data ' + id);
		var self = this;
	...
	Save: function(){
		// send script data to server
		// this.state.script.title
		// var title = prompt("Please enter you script title", "my title");
		...
		console.log("update");
			...
			$.post("script_create.php",
			  ...
			  function(res, status){
			    ...
			    if(res == 1){
			    	alert('update script successed.');
			    }else{
			    	alert('update script failed.');
			    }
			  });
		}
